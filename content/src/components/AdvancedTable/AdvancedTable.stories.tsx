...
const meta: Meta<typeof AdvancedTable> = {
  title: 'Components/DataDisplay/AdvancedTable',
  ...
  decorators: [
    (Story) => (
      <div style={{ padding: '24px', minWidth: '100%' }}>
        <Story />
      </div>
    ),
  ],
    ...
    const bulkActions = [
      {
        ...
        action: (items: Employee[]) => {
          ...
          alert(`Exporting ${items.length} selected employees`);
        },
      ...
      },
      {
        ...
        action: (items: Employee[]) => {
          ...
          alert(`Deactivating ${items.length} employees`);
        },
...
export const AllFeaturesEnabled: Story = {
  render: () => {
    ...
    const bulkActions = [
      {
        label: 'Send Email',
        action: (items: Employee[]) => alert(`Sending email to ${items.length} employees`),
        variant: 'primary' as const
      },
      {
        label: 'Export Selected',
        action: (items: Employee[]) => console.log('Export', items),
        variant: 'secondary' as const
      },
      {
        label: 'Remove',
        action: (items: Employee[]) => alert(`Removing ${items.length} employees`),
        variant: 'destructive' as const
      }
    ];
    ...
    return (
      <AdvancedTable
        ...
        subtitle="All features enabled including search, filter, export, bulk actions, and pagination"
        selectable={true}
        onRowSelect={setSelectedItems}
        ...
        render: (item) => (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ overflow: 'hidden' }}>
              <div style={{ fontWeight: 500, color: '#1f2937', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
              <div style={{ fontSize: '12px', color: '#6b7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.email}</div>
            </div>
          </div>
        )
      },
    ...
    return (
      <div>
        <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#fef3c7', border: '1px solid #f59e0b', borderRadius: '6px' }}>
          <strong>Tip:</strong> Drag the blue handles between column headers to resize columns.
        </div>
        <AdvancedTable
          data={employeeData}
          columns={resizableColumns}
          title="Resizable Columns Table"
          subtitle="Drag the dividers between column headers to resize"
