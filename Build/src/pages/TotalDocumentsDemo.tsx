...
import { Document, AuditEntry } from '../types/common';
...
const TotalDocumentsDemo: React.FC = () => {
  ...
  const columns = [
    { 
      ...
      header: 'Document ID', 
      sortable: true,
      render: (item: Document) => <span className="text-xs">{item.id}</span>
    },
    { 
      key: 'title', 
      header: 'Document Title', 
      sortable: true,
      render: (item: Document) => <span className="text-xs font-medium">{item.title}</span>
    },
    { 
      key: 'classification', 
    ...
    { 
      ...
      header: 'Access Level', 
      sortable: true,
      render: (item: Document) => <span className="text-xs">{item.accessLevel}</span>
    },
    { 
      key: 'lastModified', 
      header: 'Last Modified', 
      sortable: true,
      render: (item: Document) => <span className="text-xs">{item.lastModified}</span>
    }
  ];
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {selectedDocument.auditTrail.map((entry: AuditEntry, index: number) => (
