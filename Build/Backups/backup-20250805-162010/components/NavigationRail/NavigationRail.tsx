...
const NavigationRail: React.FC<NavigationRailProps> = ({
  ...
  return (
    <nav 
      className={clsx(
        'flex flex-col transition-all duration-300 ease-in-out h-full w-full min-h-full overflow-hidden',
        className
      )}
      style={{
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    collapsed ? '' : 'px-4',
                    isActive 
                      ? 'bg-[var(--color-active-background)] before:absolute before:top-0 before:h-full before:w-1 before:bg-[var(--color-blue-default)]' 
                      : 'hover:bg-gray-100',
                    item.disabled && 'opacity-50 cursor-not-allowed',
                    position === 'left' ? 'before:left-0' : 'before:right-0'
                  )}
                ...
                >
                  ...
                  {showTooltips && collapsed && (
                    <div className={clsx(
                      "absolute px-2 py-1 bg-gray-900 text-white text-sm rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10 max-w-xs",
