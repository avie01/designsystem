...
export interface CustomRightNavigationRailProps {
  ...
  /** Callback when an icon is clicked while collapsed (for slide-out behavior) */
...
const CustomRightNavigationRail: React.FC<CustomRightNavigationRailProps> = ({
  ...
  const handleItemClick = (item: MenuItem) => {
    if (!item.disabled) {
      if (collapsed && onIconClick) {
        // When collapsed, call the slide-out callback
        onIconClick(item);
      } else {
